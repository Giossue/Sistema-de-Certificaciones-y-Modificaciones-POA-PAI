...
type ListarLiquidacionesParams = {
  user: AuthUser;
  pageQuery?: string;
  pageSizeQuery?: string;
};

export async function listarLiquidaciones(prisma: PrismaClient, params: ListarLiquidacionesParams) {
  ...
  if (pageQuery || pageSizeQuery) {
    const [totalItems, data] = await Promise.all([
      prisma.liquidacionCertificacion.count({ where }),
    ...
    return { items: data.map(toLiquidacionItem), totalItems, page, pageSize, totalPages: totalPages(totalItems, pageSize) };
  }
  ...
  return data.map(toLiquidacionItem);
}
