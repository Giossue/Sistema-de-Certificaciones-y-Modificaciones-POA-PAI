...
type ObservarModificacionPoaParams = {
  prisma: PrismaClient;
...
export async function observarModificacionPoa(params: ObservarModificacionPoaParams) {
  ...
  await auditoriaService.registrar({ usuarioId: user.id, entidad: "ModificacionPoa", entidadId: id, accion: "OBSERVAR", estadoAnterior: current.estado, estadoNuevo: "observada", motivo: observaciones, ...auditMeta, payloadAnterior: current, payloadNuevo: { observaciones } });
